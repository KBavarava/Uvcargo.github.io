...
if(isset($_POST['submit']))
{
   ...
   $res=mysqli_query($db,"select * from branchlog where branchcity='$branchcity'");
   $count=mysqli_num_rows($res);
   if($count>0){
      if(isset($_GET['id']) && $_GET['id']!=''){
   ...
   if($msg==''){
        if(isset($_GET['id']) && $_GET['id']!=''){
          mysqli_query($db,"update branchlog set branchcity='$branchcity',branchname='$branchname',
          branchcontactnumber='$branchcontactnumber',password='$password' where id='$id'");
          }
		header('location:forms.php');
          die();
     }
